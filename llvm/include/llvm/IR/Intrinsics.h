...
#undef GET_INTRINSIC_ENUM_VALUES
  ...
  LLVM_ABI AttributeList getAttributes(LLVMContext &C, ID id);
  ...
  /// the intrinsic.
  LLVM_ABI Function *getDeclaration(Module *M, ID id,
                           ArrayRef<Type *> Tys = std::nullopt);
  ...
  LLVM_ABI int lookupLLVMIntrinsicByName(ArrayRef<const char *> NameTable,
                                StringRef Name);

  /// Map a Clang builtin name to an intrinsic ID.
  LLVM_ABI ID getIntrinsicForClangBuiltin(const char *Prefix, StringRef BuiltinName);

  /// Map a MS builtin name to an intrinsic ID.
  LLVM_ABI ID getIntrinsicForMSBuiltin(const char *Prefix, StringRef BuiltinName);
