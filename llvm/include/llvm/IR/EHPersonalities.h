...
/// Unknown.
LLVM_C_ABI EHPersonality classifyEHPersonality(const Value *Pers);

LLVM_C_ABI StringRef getEHPersonalityName(EHPersonality Pers);

LLVM_C_ABI EHPersonality getDefaultEHPersonality(const Triple &T);
...
LLVM_C_ABI bool canSimplifyInvokeNoUnwind(const Function *F);
...
/// expensive.
LLVM_C_ABI DenseMap<BasicBlock *, ColorVector> colorEHFunclets(Function &F);
