...
/// \c raw_ostream.
LLVM_C_ABI ModulePass *createPrintModulePass(raw_ostream &OS,
                                  const std::string &Banner = "",
                                  bool ShouldPreserveUseListOrder = false);

/// Create and return a pass that prints functions to the specified
/// \c raw_ostream as they are processed.
LLVM_C_ABI FunctionPass *createPrintFunctionPass(raw_ostream &OS,
                                      const std::string &Banner = "");
...
LLVM_C_ABI void printLLVMNameWithoutPrefix(raw_ostream &OS, StringRef Name);

/// Return true if a pass is for IR printing.
LLVM_C_ABI bool isIRPrintingPass(Pass *P);
