...
LLVM_ABI APInt getIConstantFromReg(Register VReg, const MachineRegisterInfo &MRI);
...
LLVM_READNONE
LLT LLVM_ABI getLCMType(LLT OrigTy, LLT TargetTy);

LLVM_READNONE
/// Return smallest type that covers both \p OrigTy and \p TargetTy and is
/// multiple of TargetTy.
LLT LLVM_ABI getCoverTy(LLT OrigTy, LLT TargetTy);
...
LLVM_READNONE
LLT LLVM_ABI getGCDType(LLT OrigTy, LLT TargetTy);
...
LLVM_ABI bool shouldOptForSize(const MachineBasicBlock &MBB, ProfileSummaryInfo *PSI,
