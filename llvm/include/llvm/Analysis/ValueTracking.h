...
LLVM_ABI bool isKnownNonZero(const Value *V, const DataLayout &DL, unsigned Depth = 0,
                    AssumptionCache *AC = nullptr,
                    const Instruction *CxtI = nullptr,
                    const DominatorTree *DT = nullptr,
                    bool UseInstrInfo = true);
...
///   x > +0 --> true
///   x < -0 --> false
LLVM_ABI bool SignBitMustBeZero(const Value *V, const DataLayout &DL,
                       const TargetLibraryInfo *TLI);
...
LLVM_ABI Value *FindInsertedValue(Value *V, ArrayRef<unsigned> idx_range,
